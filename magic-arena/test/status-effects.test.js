...
// 部署后将玩家单位移到 x=7（敌方在 x=10，技能 range=3，|7-10|=3 恰好覆盖）
function setupProximity() {
  ['炎法师·艾拉','雷法师·特斯拉','暗法师·莫甘娜','风行者·翠影'].forEach(n => Game._testSetUnitPos(n, 7, null));
}
...
setupProximity(); // 玩家移近至 x=7（敌方 x=10，沉默 range=3 恰好覆盖）
// 特斯拉(7,3) 直接施放沉默术于 (10,3) 维克
castAt(7, 3, 1, 10, 3);
const s1target = enemyAt(10, 3);
...
Game.startCampaign(1);
setupProximity();
// 艾拉(7,1) 直接施放灼烧术于 (10,1) 卡尔
castAt(7, 1, 1, 10, 1);
const s2target = enemyAt(10, 1);
...
Game.startCampaign(1);
setupProximity();
castAt(7, 5, 2, 10, 5); // 莫甘娜技能槽[2] = 致盲术，目标 (10,5)
const s3target = enemyAt(10, 5);
...
Game.startCampaign(1);
setupProximity();
castAt(7, 5, 1, 10, 5); // 莫甘娜技能槽[1] = 易伤术，目标 (10,5)
const s6target = enemyAt(10, 5);
