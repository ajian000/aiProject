...
| # | 方向 | 说明 | 质量期望（不是指标，是底线） |
|---|---|---|---|
| 1 | **核心玩法**（Core Gameplay） | 战斗系统、技能机制、单位行为、战场机制、敌方 AI | 新技能/状态/AI 行为应有**独立逻辑实现**，不是复制既有函数改个参数就叫新机制。给玩家带来可感知的新战术选择。 |
| 2 | **内容扩建**（Content Buildout） | 阵营、单位、技能、地图、战役关卡、游戏模式的数据量与多样性 | **禁止 clone + rename**。新增内容必须给游戏带来**新的玩法差异**，而非换个名字和技能就提交。纯数据表添加不算独立推进——必须伴随至少一处核心逻辑变更。 |
| 3 | **系统新创**（System Innovation） | 提出并实现全新子系统，给游戏增加新维度 | 新子系统应是**独立的玩法维度**（装备/天赋/成就/商店/等级等），需要新增数据结构、UI 组件和核心逻辑，不是简单挂一个面板。 |
| 4 | **体验打磨**（Experience Polish） | 品质、平衡性、手感、UI/UX、性能、动画反馈 | 可量化的改善（帧率提升、操作步骤减少、反应速度提升）。仅调色/改字体不算推进。 |
| 5 | **工程基石**（Engineering Foundation） | 可测性、代码质量、构建工具链、文档完整性 | 功能性改进（新增测试用例、修复隐患、代码重构）。仅格式化/注释/重命名不算推进。 |
...
3. 方向推进承诺（Directional Progress）：**这次调用我必须在至少一个开发方向（§2.5）上产生推进。** 自问：当前的游戏最需要什么？——核心玩法要深化？内容要扩建？需要新子系统？体验要打磨？工程要加固？选定 1~n 个方向，认领对应任务并执行。不得以"方向上都有人做"为由跳过本步——即使所有方向都有活跃 Agent，你也可以在**工程基石**方向上做贡献（测试/代码质量/文档/构建）。LOG 标注本次推进的方向标签（如 `方向1:核心玩法` / `方向3:系统新创`）。
   - **禁止 clone + rename**。如果把现有阵营/单位/技能直接复制一份、改个名字和数值就交上来，这不算有意义的推进。每次推进应给游戏带来**玩家能感知的新体验、新机制或新挑战**。
   - 方向 2（内容扩建）的推进必须伴随至少一处**核心逻辑变更**（新增函数/新机制/新交互）。纯数据表添加不算独立推进。
   - 方向 4/5 的推进应有**可验证的量化改善**（耗时减少 N%、操作步骤缩短等）。
   - 若无 Backlog 中有价值的任务，自行提出并写入。
   不满足于琐碎改动。**问自己：这个贡献让玩家玩起来和之前有什么不同？如果答不上来，说明力度不够。**
...
10. 自我审查（Self-Review）：标 Done 前，回答三个问题：
   - ① 本次推进在哪个方向上产生了贡献？
