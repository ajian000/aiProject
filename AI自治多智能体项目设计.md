...
| 2 | **内容扩建** | 🟢 **最高优先级** | 章节战役、兵种体系、战场规模、新单位、剧情叙事、世界百科 | 游戏当前 <5 分钟。**每次调用优先做方向2**：扩充章节(≥15关)、扩大战场(≥10×10)、增加单位数(≥4v4)、引入兵种克制。每次扩建必须可见增加游戏时长。 |
...
**推进原则（更新版 · 基于质量评估）**
- **方向1 冻结**：禁止任何方向1新增工作。仅接受已存在机制的 bug 修复。
- **每次调用必须优先做方向2**：章节数够了吗？战场够大吗？单位数够了吗？兵种有了吗？方向2没有明显可推进的再考虑方向3/5。
...
3. 方向推进承诺（Directional Progress）：**这次调用我必须在至少一个开发方向（§2.5）上产生推进。⚠️ 注意：方向1（核心玩法）已冻结，禁止新增任何技能/状态/机制。至少有一半的调用应优先做方向2（内容扩建）。**
   自问：当前游戏时长 <5 分钟，最需要什么？
   - **章节够多吗？**（目标 Phase 0: ≥12 关 / Phase 1: ≥15 关）
   - **战场够大吗？**（目标 Phase 0: 10×10 / Phase 1: 12×10）
   ...
   - 方向2的推进目标：**每次调用至少使游戏预估时长增加 1 分钟**（通过加章节、扩战场、增单位、或引入新决策维度）。
